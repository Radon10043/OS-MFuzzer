...
LastEditors  : Radon
LastEditTime : 2025-07-28 14:14:41
...
def loop(identifier: OpenAI | Anthropic | GoogleAI, calibrator: OpenAI | Anthropic | GoogleAI, config: dict):
...
def main2(config: dict):
    # TODO: Finish this function
    iden_llm = init_chat_model(model=config["identifier"]["model"], model_provider=config["identifier"]["provider"])
    cali_llm = init_chat_model(model=config["calibrator"]["model"], model_provider=config["calibrator"]["provider"])

    # Set Iden llm System Prompt Template & Cali llm System Prompt Template
    ispt = Path(config["identifier"]["prompts"]["system"]).read_text(encoding="utf-8")
    cspt = Path(config["calibrator"]["prompts"]["system"]).read_text(encoding="utf-8")

    ACTF("Starting the discussion between iden llm and cali llm ...")
    max_iter = config.get("max_iter", 10)
    it = 0
    while it < max_iter:
        ACTF(f"Iteration {it + 1}/{max_iter}")
        ACTF(f"Asking iden llm {iden_llm} ...")
