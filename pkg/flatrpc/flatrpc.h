...
struct ExecResultRaw FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  const flatbuffers::Vector<uint64_t> *bincover() const {
    return GetPointer<const flatbuffers::Vector<uint64_t> *>(VT_BINCOVER);
  }
  const flatbuffers::Vector<uint64_t> *binsignals() const {
    return GetPointer<const flatbuffers::Vector<uint64_t> *>(VT_BINSIGNALS);
  }
  bool hanged() const {
  ...
  bool Verify(flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
           ...
           VerifyOffset(verifier, VT_BINCOVER) &&
           verifier.VerifyVector(bincover()) &&
           VerifyOffset(verifier, VT_BINSIGNALS) &&
           verifier.VerifyVector(binsignals()) &&
...
struct ExecResultRawBuilder {
  ...
  void add_bincover(flatbuffers::Offset<flatbuffers::Vector<uint64_t>> bincover) {
    fbb_.AddOffset(ExecResultRaw::VT_BINCOVER, bincover);
  }
  void add_binsignals(flatbuffers::Offset<flatbuffers::Vector<uint64_t>> binsignals) {
    fbb_.AddOffset(ExecResultRaw::VT_BINSIGNALS, binsignals);
  }
...
inline flatbuffers::Offset<ExecResultRaw> CreateExecResultRaw(
  ...
  builder_.add_error(error);
  builder_.add_binsignals(binsignals);
  builder_.add_bincover(bincover);
...
inline flatbuffers::Offset<ExecResultRaw> CreateExecResultRawDirect(
    ...
    const std::vector<uint64_t> *bincover = nullptr,
    const std::vector<uint64_t> *binsignals = nullptr,
    bool hanged = false,
    const char *error = nullptr,
    flatbuffers::Offset<rpc::ProgInfoRaw> info = 0) {
  auto output__ = output ? _fbb.CreateVector<uint8_t>(*output) : 0;
  auto bincover__ = bincover ? _fbb.CreateVector<uint64_t>(*bincover) : 0;
  auto binsignals__ = binsignals ? _fbb.CreateVector<uint64_t>(*binsignals) : 0;
  auto error__ = error ? _fbb.CreateString(error) : 0;
...
inline ExecResultRawT &ExecResultRawT::operator=(ExecResultRawT o) FLATBUFFERS_NOEXCEPT {
  ...
  std::swap(bincover, o.bincover);
  std::swap(binsignals, o.binsignals);
...
inline void ExecResultRaw::UnPackTo(ExecResultRawT *_o, const flatbuffers::resolver_function_t *_resolver) const {
  ...
  { auto _e = bincover(); if (_e) { _o->bincover.resize(_e->size()); for (flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) { _o->bincover[_i] = _e->Get(_i); } } }
  { auto _e = binsignals(); if (_e) { _o->binsignals.resize(_e->size()); for (flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) { _o->binsignals[_i] = _e->Get(_i); } } }
...
inline flatbuffers::Offset<ExecResultRaw> CreateExecResultRaw(flatbuffers::FlatBufferBuilder &_fbb, const ExecResultRawT *_o, const flatbuffers::rehasher_function_t *_rehasher) {
  ...
  auto _bincover = _o->bincover.size() ? _fbb.CreateVector(_o->bincover) : 0;
  auto _binsignals = _o->binsignals.size() ? _fbb.CreateVector(_o->binsignals) : 0;
