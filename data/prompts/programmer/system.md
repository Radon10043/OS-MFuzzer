...
- The encoded metamorphic relation (including the preliminary preparation and the later cleanup) should be placed in the `syz_mr` function for other functions to call, and the return type of `syz_mr` function must be `static long`.
- The source input needs to be set as a parameter of the `syz_mr` function, and the parameter's type must be `volatile long`.
