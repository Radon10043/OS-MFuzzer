...
### This Page
  ...
  * [Show Source](../_sources/infiniband/user_mad.rst.txt)

English

  * [Chinese (Simplified)](../translations/zh_CN/infiniband/user_mad.html)

# Userspace MAD access¶

## Device files¶

> Each port of each InfiniBand device has a “umad” device and an “issm” device
> attached. For example, a two-port HCA will have two umad devices and two
> issm devices, while a switch will have one device of each type (for switch
> port 0).
...
> the errno is set to ENOSPC and the length of the buffer needed is set in
> mad.length.
>
...
> order) in MADs being sent to match request/response pairs. The upper 32 bits
> are reserved for use by the kernel and will be overwritten before a MAD is
> sent.
...
> cleared when the issm file is closed. No read, write or other operations can
> be performed on the issm file.
...
(C)The kernel development community. | Powered by [Sphinx 5.0.1](http://sphinx-doc.org/) & [Alabaster 0.7.12](https://github.com/bitprophet/alabaster) | [Page source](../_sources/infiniband/user_mad.rst.txt)
