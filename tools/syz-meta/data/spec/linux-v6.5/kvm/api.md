...
Queues an external interrupt to be injected. This ioctl is overleaded with 3
...
and passed to userspace through KVM_EXIT_XEN. In this ase, all of the blob
...
RISC-V core registers represent the general excution state of a Guest VCPU and
it has the following id bit patterns:
...
increasing order, a lookup can stop when encoutering such an entry.
...
scalable archietctural SVE registers KVM_REG_ARM64_SVE_ZREG(),
...
On succsful completion, the pending HPT will become the guest's active HPT and
...
protected VM had been prepared for asynchonous teardown with
...
but but other fields cannot change for a given sending port. A port mapping is
...
guest to interact with host userpace.
...
(KVM_VM_S390_UNCONTROL) on it's host page table that cannot be resolved by the
...
KVM is capable of tracking dirty memory using ring buffers that are mmaped
into userspace; there is one dirty ring per vcpu.

The dirty ring is available to userspace as an array of `struct
kvm_dirty_gfn`. Each dirty entry it's defined as:
...
To harvest the dirty pages, userspace accesses the mmaped ring buffer to read
...
implmented Hyper-V features are provided unconditionally when Hyper-V
